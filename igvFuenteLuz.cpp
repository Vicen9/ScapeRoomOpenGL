...
void igvFuenteLuz::aplicar(void) {
	...
	if (esta_encendida())
	{
		...
		glLightfv(idLuz, GL_POSITION, posicion.cloneToFloatArray()); // POSICION

		glLightfv(idLuz, GL_AMBIENT, colorAmbiente.cloneToFloatArray()); // COLOR AMBIENTAL
		glLightfv(idLuz, GL_DIFFUSE, colorDifuso.cloneToFloatArray()); // COLOR DIFUSO
		glLightfv(idLuz, GL_SPECULAR, colorEspecular.cloneToFloatArray()); // COLOR ESPECULAR

		glLightf(idLuz, GL_CONSTANT_ATTENUATION, aten_a0);
		glLightf(idLuz, GL_LINEAR_ATTENUATION, aten_a1);
		glLightf(idLuz, GL_QUADRATIC_ATTENUATION, aten_a2); // ATENUACION RADIAL

		glLightfv(idLuz, GL_SPOT_DIRECTION, direccion_foco.cloneToFloatArray()); // DIRECCION FOCO
		glLightf(idLuz, GL_SPOT_CUTOFF, angulo_foco); // ANGULO FOCO
		glLightf(idLuz, GL_SPOT_EXPONENT, exponente_foco); // ATENUACION ANGULAR
